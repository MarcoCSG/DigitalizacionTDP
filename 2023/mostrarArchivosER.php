...
<body>
    ...
    $conexion = mysqli_connect("localhost", "root", "", "tdp23");
    ...
    ?>
        ...
        <!-- Mensaje de resumen -->
        <p>En el área de <strong><?php echo htmlspecialchars($area); ?></strong>, en el rubro de <strong><?php echo htmlspecialchars($clasificacion); ?></strong>.</p>

        <?php
        // Paso 3: Construir la consulta SQL sin LIMIT
        $consultaTotal = "SELECT SUM(cantidad_folios) AS total_folios FROM $area WHERE clasificacion = '$clasificacion'";
        ...
        $consulta = "SELECT * FROM $area WHERE clasificacion = '$clasificacion'";
        ...
        <div class="paginacion">
            ...
            $totalFilas = mysqli_num_rows(mysqli_query($conexion, "SELECT * FROM $area WHERE clasificacion = '$clasificacion'" . (!empty($search) ? " AND nombre_archivo LIKE '%$search%'" : "")));
            ...
            for ($i = 1; $i <= $totalPaginas; $i++) {
                ...
                echo "<a class='$claseSeleccionada' href='?clasificacion=$clasificacion&area=$area&search=" . urlencode($search) . "&pagina=$i'>$i</a> ";
            }
            ?>
        </div>
