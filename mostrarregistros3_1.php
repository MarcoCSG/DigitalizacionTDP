...
$query = "SELECT 
            ...
            formatos f
            JOIN 
            formato_3_1 f31 ON f.id = f31.formato_id
            JOIN 
...
// Obtener el no_acta de registros para calcular el número de páginas
$query_count = "SELECT COUNT(*) as no_acta 
                FROM formatos f
...
if (!empty($search)) {
    $query_count .= " AND (f31.no LIKE ? OR f31.denominacion LIKE ?)";
    $types_count .= "ss";
...
$no_acta_registros = $result_count->fetch_assoc()['no_acta'];
$no_acta_paginas = ceil($no_acta_registros / $registros_por_pagina);
...
<div class="search-container">
        <form method="get" action="mostrarregistros3_1.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td>" . htmlspecialchars($row['no_acta']) . "</td>
                    <td>" . htmlspecialchars($row['fecha_aprobacion']) . "</td>
                    <td>" . htmlspecialchars($row['no_gaceta']) . "</td>
                    <td>" . htmlspecialchars($row['fecha_publicacion']) . "</td>
                    <td>" . htmlspecialchars($row['observaciones_3_1']) . "</td>
                    <td>" . htmlspecialchars($row['informacion_al']) . "</td>
                    <td>" . htmlspecialchars($row['responsable']) . "</td>
                    <td class='acciones'>
                        <a href='php/editarRegistro3_1.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro3_1.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
        ...
        if ($no_acta_paginas > 1) {
            ...
            $fin = min($no_acta_paginas, $pagina_actual + $limite);
            ...
            if ($fin < $no_acta_paginas) {
                echo "<span>...</span>";
            }

            // Enlace Siguiente, Último
            if ($pagina_actual < $no_acta_paginas) {
                ...
                echo "<a href='?pagina=" . $no_acta_paginas . "&search=" . urlencode($search) . "&anio=" . urlencode($anio) . "&area=" . urlencode($area_nombre) . "&clasificacion=" . urlencode($clasificacion_codigo) . "'>Último &raquo;</a>";
            }
