...
$query = "SELECT 
            ...
            formatos f
            JOIN 
            formato_3_2 f32 ON f.id = f32.formato_id
            JOIN 
...
if (!empty($search)) {
    $query .= " AND (f32.no LIKE ? OR f32.libro LIKE ?)";
...
// Obtener el acta_del de registros para calcular el número de páginas
$query_count = "SELECT COUNT(*) as acta_del 
                FROM formatos f
                JOIN formato_3_2 f32 ON f.id = f32.formato_id
...
if (!empty($search)) {
    $query_count .= " AND (f32.no LIKE ? OR f32.libro LIKE ?)";
    $types_count .= "ss";
...
$acta_del_registros = $result_count->fetch_assoc()['acta_del'];
$acta_del_paginas = ceil($acta_del_registros / $registros_por_pagina);
...
<div class="search-container">
        <form method="get" action="mostrarregistros3_2.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
    ...
    <div id="modalPDFTDP" class="modal">
        <div class="modal-content">
            ...
            <label for="observacionesTDP">LIBROS ANTERIORES:
                <span class="tooltip">?
                    <span class="tooltip-text">La ubicación física o el lugar en donde se encuentran los Libros de las administraciones municipales anteriores.</span>
                </span>
            </label>
    ...
    <div id="modalPDFMunicipio" class="modal">
        <div class="modal-content">
            ...
            <label for="observacionesMunicipio">LIBROS ANTERIORES::
                <span class="tooltip">?
                    <span class="tooltip-text">La ubicación física o el lugar en donde se encuentran los Libros de las administraciones municipales anteriores.</span>
                </span>
            </label>
    ...
    <script>
        ...
        function generarPDFMunicipio() {
            ...
            const url = `php/generarPDF3_2_municipio.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&observaciones=${encodeURIComponent(observaciones)}&opcion=${encodeURIComponent(opcionSeleccionada)}`;
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td>" . htmlspecialchars($row['acta_del']) . "</td>
                    <td>" . htmlspecialchars($row['acta_al']) . "</td>
                    <td>" . htmlspecialchars($row['fojas']) . "</td>
                    <td>" . htmlspecialchars($row['observaciones_3_2']) . "</td>
                    <td>" . htmlspecialchars($row['informacion_al']) . "</td>
                    <td>" . htmlspecialchars($row['responsable']) . "</td>
                    <td class='acciones'>
                        <a href='php/editarRegistro3_2.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro3_2.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
        ...
        if ($acta_del_paginas > 1) {
            ...
            $fin = min($acta_del_paginas, $pagina_actual + $limite);
            ...
            if ($fin < $acta_del_paginas) {
                echo "<span>...</span>";
            }

            // Enlace Siguiente, Último
            if ($pagina_actual < $acta_del_paginas) {
                ...
                echo "<a href='?pagina=" . $acta_del_paginas . "&search=" . urlencode($search) . "&anio=" . urlencode($anio) . "&area=" . urlencode($area_nombre) . "&clasificacion=" . urlencode($clasificacion_codigo) . "'>Último &raquo;</a>";
            }
