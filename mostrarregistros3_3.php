...
$query = "SELECT 
            ...
            formatos f
            JOIN 
            formato_3_3 f33 ON f.id = f33.formato_id
            JOIN 
...
if (!empty($search)) {
    $query .= " AND (f33.no LIKE ? OR f33.acta LIKE ?)";
...
$query_count = "SELECT COUNT(*) as fecha 
                FROM formatos f
                JOIN formato_3_3 f33 ON f.id = f33.formato_id
...
if (!empty($search)) {
    $query_count .= " AND (f33.no LIKE ? OR f33.acta LIKE ?)";
    $types_count .= "ss";
...
<div class="search-container">
        <form method="get" action="mostrarregistros3_3.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
    ...
    <script>
        ...
        function generarPDFMunicipio() {
            ...
            const url = `php/generarPDF3_3_municipio.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&observaciones=${encodeURIComponent(observaciones)}&opcion=${encodeURIComponent(opcionSeleccionada)}`;
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td>" . htmlspecialchars($row['acta']) . "</td>
                    <td>" . htmlspecialchars($row['fecha']) . "</td>
                    <td>" . htmlspecialchars($row['acuerdo']) . "</td>
                    <td>" . htmlspecialchars($row['responsable_cabildo']) . "</td>
                    <td>" . htmlspecialchars($row['estado']) . "</td>
                    <td>" . htmlspecialchars($row['observaciones3_3']) . "</td>
                    <td>" . htmlspecialchars($row['informacion_al']) . "</td>
                    <td>" . htmlspecialchars($row['responsable']) . "</td>
                    <td class='acciones'>
                        <a href='php/editarRegistro3_3.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro3_3.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
