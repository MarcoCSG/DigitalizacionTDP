...
$query = "SELECT 
            ...
            formatos f
            JOIN 
            formato_3_6 f36 ON f.id = f36.formato_id
            JOIN 
...
if (!empty($search)) {
    $query .= " AND (f36.no LIKE ? OR f36.tipo LIKE ?)";
...
// Obtener el fecha de registros para calcular el número de páginas
$query_count = "SELECT COUNT(*) as fecha 
                FROM formatos f
                JOIN formato_3_6 f36 ON f.id = f36.formato_id
...
if (!empty($search)) {
    $query_count .= " AND (f36.no LIKE ? OR f36.tipo LIKE ?)";
    $types_count .= "ss";
...
$fecha_registros = $result_count->fetch_assoc()['fecha'];
$fecha_paginas = ceil($fecha_registros / $registros_por_pagina);
...
<div class="search-container">
        <form method="get" action="mostrarregistros3_6.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
    ...
    <script>
        ...
        function generarPDFMunicipio() {
            ...
            const url = `php/generarPDF3_6_municipio.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&observaciones=${encodeURIComponent(observaciones)}&opcion=${encodeURIComponent(opcionSeleccionada)}`;
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td>" . htmlspecialchars($row['promovido_ante']) . "</td>
                    <td>" . htmlspecialchars($row['en_contra_de']) . "</td>
                    <td>" . htmlspecialchars($row['descripcion']) . "</td>
                    <td>" . htmlspecialchars($row['estado_juridico']) . "</td>
                    <td>" . htmlspecialchars($row['acciones_inmediatas']) . "</td>
                    <td>" . htmlspecialchars($row['informacion_al']) . "</td>
                    <td>" . htmlspecialchars($row['responsable']) . "</td>
                    <td class='acciones'>
                        <a href='php/editarRegistro3_6.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro3_6.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
        ...
        if ($fecha_paginas > 1) {
            ...
            $fin = min($fecha_paginas, $pagina_actual + $limite);
            ...
            if ($fin < $fecha_paginas) {
                echo "<span>...</span>";
            }

            // Enlace Siguiente, Último
            if ($pagina_actual < $fecha_paginas) {
                ...
                echo "<a href='?pagina=" . $fecha_paginas . "&search=" . urlencode($search) . "&anio=" . urlencode($anio) . "&area=" . urlencode($area_nombre) . "&clasificacion=" . urlencode($clasificacion_codigo) . "'>Último &raquo;</a>";
            }
