...
$query = "SELECT 
            ...
            formatos f
            JOIN 
            formato_3_8 f38 ON f.id = f38.formato_id
            JOIN 
...
// Inicializar conceptos y parámetros para bind_param
...
if (!empty($search)) {
    $query .= " AND (f38.no LIKE ? OR f38.concepto LIKE ?)";
...
if (!empty($search)) {
    $query_count .= " AND (f38.no LIKE ? OR f38.concepto LIKE ?)";
    $types_count .= "ss";
...
<div class="search-container">
        <form method="get" action="mostrarregistros3_8.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
    ...
    <script>
        ...
        function generarPDFMunicipio() {
            ...
            const url = `php/generarPDF3_8_municipio.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&observaciones=${encodeURIComponent(observaciones)}&opcion=${encodeURIComponent(opcionSeleccionada)}`;
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td class='acciones'>
                        <a href='php/editarRegistro3_8.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro3_8.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
