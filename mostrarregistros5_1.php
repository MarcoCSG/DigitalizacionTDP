...
$query = "SELECT 
          ...
          FROM 
            formatos f
          JOIN 
            formato_5_1 f51 ON f.id = f51.formato_id
          JOIN 
...
if (!empty($search)) {
    $query .= " AND (f51.articulo LIKE ? OR f51.costo_unitario LIKE ?)";
...
$query_count = "SELECT COUNT(*) as total 
                FROM formatos f
                JOIN formato_5_1 f51 ON f.id = f51.formato_id
...
if (!empty($search)) {
    $query_count .= " AND (f51.articulo LIKE ? OR f51.costo_unitario LIKE ?)";
    $types_count .= "ss";
...
<div class="search-container">
        <form method="get" action="mostrarregistros5_1.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
...
function generarPDFTDP() {
    ...
    const url = `php/generarPDF5_1.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&superviso=${encodeURIComponent(superviso)}&observaciones=${encodeURIComponent(observaciones)}`;
...
function generarPDFMunicipio() {
    ...
    const url = `php/generarPDF5_1_municipio.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&observaciones=${encodeURIComponent(observaciones)}`;
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td class='acciones'>
                        <a href='php/editarRegistro5_1.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro5_1.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
