...
$query = "SELECT 
          ...
          FROM 
            formatos f
          JOIN 
            formato_5_12 f512 ON f.id = f512.formato_id
          JOIN 
...
if (!empty($search)) {
    $query .= " AND (f512.unidad_administrativa LIKE ? OR f512.ubicacion LIKE ?)";
...
if (!empty($search)) {
    $query_count .= " AND (f512.unidad_administrativa LIKE ? OR f512.observaciones LIKE ?)";
    $types_count .= "ss";
...
<div class="search-container">
        <form method="get" action="mostrarregistros5_12.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
    ...
    <script>
        ...
        function generarPDFTDP() {
            ...
            // Si no se selecciona ninguna opción, asignar un vigencia_documental predeterminado vacío
        ...
        function generarPDFMunicipio() {
            ...
            const url = `php/generarPDF5_12_municipio.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&observaciones=${encodeURIComponent(observaciones)}&opcion=${encodeURIComponent(opcionSeleccionada)}`;
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<table>
                <thead>
                    <tr>
                        ...
                        <th>CLAVE</th>
                        <th>DESCRIPCIÓN DEL CONTENIDO DE LA SERIE</th>
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td>" . htmlspecialchars($row['unidad_administrativa']) . "</td>
                    ...
                    <td class='acciones'>
                        <a href='php/editarRegistro5_12.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro5_12.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
