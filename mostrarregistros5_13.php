...
$query = "SELECT 
          ...
          FROM 
            formatos f
          JOIN 
            formato_5_13 f513 ON f.id = f513.formato_id
          JOIN 
...
if (!empty($search)) {
    $query .= " AND (f513.unidad_administrativa LIKE ? OR f513.ubicacion LIKE ?)";
...
$query_count = "SELECT COUNT(*) as total 
                FROM formatos f
                JOIN formato_5_13 f513 ON f.id = f513.formato_id
...
if (!empty($search)) {
    $query_count .= " AND (f513.unidad_administrativa LIKE ? OR f513.observaciones LIKE ?)";
    $types_count .= "ss";
...
<div class="search-container">
        <form method="get" action="mostrarregistros5_13.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
    ...
    <script>
        ...
        function generarPDFTDP() {
            ...
            // Si no se selecciona ninguna opción, asignar un observaciones predeterminado vacío
        ...
        function generarPDFMunicipio() {
            ...
            const url = `php/generarPDF5_13_municipio.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&observaciones=${encodeURIComponent(observaciones)}&opcion=${encodeURIComponent(opcionSeleccionada)}`;
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td class='acciones'>
                        <a href='php/editarRegistro5_13.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro5_13.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
