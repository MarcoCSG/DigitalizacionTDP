...
$query = "SELECT 
          ...
          FROM 
            formatos f
          JOIN 
            formato_5_14 f514 ON f.id = f514.formato_id
          JOIN 
...
if (!empty($search)) {
    $query .= " AND (f514.informacion LIKE ? OR f514.ubicacion LIKE ?)";
...
$query_count = "SELECT COUNT(*) as total 
                FROM formatos f
                JOIN formato_5_14 f514 ON f.id = f514.formato_id
...
if (!empty($search)) {
    $query_count .= " AND (f514.informacion LIKE ? OR f514.observaciones LIKE ?)";
    $types_count .= "ss";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td class='acciones'>
                        <a href='php/editarRegistro5_14.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro5_14.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
