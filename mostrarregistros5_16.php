...
$query = "SELECT 
        ...
        FROM 
            formatos f
        JOIN 
            formato_5_16 f516 ON f.id = f516.formato_id
        JOIN 
...
if (!empty($search)) {
    $query_count .= " AND (f516.inventario LIKE ? OR f516.responsable LIKE ?)";
    $types_count .= "ss";
...
<body>
    ...
    <div class="search-container">
        <form method="get" action="mostrarregistros5_16.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
    ...
    <script>
        ...
        function generarPDFMunicipio() {
            ...
            const url = `php/generarPDF5_16_municipio.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&observaciones=${encodeURIComponent(observaciones)}&opcion=${encodeURIComponent(opcionSeleccionada)}`;
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<table>
                <thead>
                    <tr>
                        ...
                        <th>ÁREA</th>
                        <th>RESPONSABLE</th>
                        <th>COMBINACIÓN SI</th>
                        <th>COMBINACIÓN NO</th>
                        <th>OBSERVACIONES</th>
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td>" . htmlspecialchars($row['area516']) . "</td>
                    <td>" . htmlspecialchars($row['responsable_516']) . "</td>
                    <td>" . htmlspecialchars($row['combinacion_si']) . "</td>
                    <td>" . htmlspecialchars($row['combinacion_no']) . "</td>
                    <td>" . htmlspecialchars($row['observaciones']) . "</td>
                    <td>" . htmlspecialchars($row['informacion_al']) . "</td>
                    <td>" . htmlspecialchars($row['responsable']) . "</td>
                    <td class='acciones'>
                        <a href='php/editarRegistro5_16.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro5_16.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
