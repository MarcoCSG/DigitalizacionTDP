...
$query = "SELECT 
          ...
          FROM 
            formatos f
          JOIN 
            formato_5_18 f18 ON f.id = f18.formato_id
          JOIN 
...
if (!empty($search)) {
    $query .= " AND (f18.clave LIKE ? OR f18.lugar_movilidad_equipo LIKE ?)";
...
if (!empty($search)) {
    $query_count .= " AND (f18.clave LIKE ? OR f18.lugar_movilidad_equipo LIKE ?)";
    $types_count .= "ss";
...
<div class="search-container">
        <form method="get" action="mostrarregistros5_18.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td>" . htmlspecialchars($row['lugar_movilidad_equipo']) . "</td>
                    <td>" . htmlspecialchars($row['cantidad']) . "</td>
                    <td>" . htmlspecialchars($row['en_poder']) . "</td>
                    <td>" . htmlspecialchars($row['cantidad_copias']) . "</td>
                    <td>" . htmlspecialchars($row['informacion_al']) . "</td>
                    <td>" . htmlspecialchars($row['responsable']) . "</td>
                    <td class='acciones'>
                        <a href='php/editarRegistro5_18.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro5_18.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
