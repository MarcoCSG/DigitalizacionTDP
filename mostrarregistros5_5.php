...
$query = "SELECT 
          ...
          FROM 
            formatos f
          JOIN 
            formato_5_5 f55 ON f.id = f55.formato_id
          JOIN 
...
if (!empty($search)) {
    $query .= " AND (f55.clasificiacion_activo LIKE ? OR f55.observaciones LIKE ?)";
...
$query_count = "SELECT COUNT(*) as total 
                FROM formatos f
                JOIN formato_5_5 f55 ON f.id = f55.formato_id
...
if (!empty($search)) {
    $query_count .= " AND (f55.clasificiacion_activo LIKE ? OR f55.observaciones LIKE ?)";
    $types_count .= "ss";
...
<div class="search-container">
        <form method="get" action="mostrarregistros5_5.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
...
function generarPDFTDP() {
    ...
    const url = `php/generarPDF5_5.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&superviso=${encodeURIComponent(superviso)}&observaciones=${encodeURIComponent(observaciones)}`;
...
function generarPDFMunicipio() {
    ...
    const url = `php/generarPDF5_5_municipio.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&observaciones=${encodeURIComponent(observaciones)}`;
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td>" . htmlspecialchars($row['clasificiacion_activo']) . "</td>
                    ...
                    <td class='acciones'>
                        <a href='php/editarRegistro5_5.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro5_5.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
