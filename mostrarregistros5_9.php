...
$query = "SELECT 
          ...
          FROM 
            formatos f
          JOIN 
            formato_5_9 f59 ON f.id = f59.formato_id
          JOIN 
...
if (!empty($search)) {
    $query .= " AND (f59.informacion LIKE ? OR f59.observaciones LIKE ?)";
...
$query_count = "SELECT COUNT(*) as total 
                FROM formatos f
                JOIN formato_5_9 f59 ON f.id = f59.formato_id
...
if (!empty($search)) {
    $query_count .= " AND (f59.informacion LIKE ? OR f59.observaciones LIKE ?)";
    $types_count .= "ss";
...
<div class="search-container">
        <form method="get" action="mostrarregistros5_9.php"> <!-- Cambiar a un archivo específico para 5_11 si es necesario -->
   ...
   <div class="imprimir-pdf-btn">
    ...
    <script>
        ...
        function generarPDFMunicipio() {
            ...
            const url = `php/generarPDF5_9_municipio.php?search=${encodeURIComponent(search)}&anio=${encodeURIComponent(anio)}&area=${encodeURIComponent(area)}&clasificacion=${encodeURIComponent(clasificacion)}&elaboro=${encodeURIComponent(elaboro)}&autorizo=${encodeURIComponent(autorizo)}&observaciones=${encodeURIComponent(observaciones)}&opcion=${encodeURIComponent(opcionSeleccionada)}`;
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td class='acciones'>
                        <a href='php/editarRegistro5_9.php?id=" . urlencode($row['formato_id']) . "&$query_string'>Editar</a> | 
                        <a href='php/eliminarRegistro5_9.php?id=" . urlencode($row['formato_id']) . "&$query_string' class='boton-eliminar' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este registro?\");'> Eliminar</a>
                    </td>
                    </tr>";
        }
        echo "</tbody></table>";
