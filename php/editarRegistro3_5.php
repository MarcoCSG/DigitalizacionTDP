...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (count($errores) === 0) {
        ...
        if ($stmt_update->execute()) {
            ...
            header("Location: ../mostrarRegistros3_5.php?mensaje=Registro%20actualizado%20correctamente&area=" . urlencode($area) . "&clasificacion=" . urlencode($clasificacion) . "&anio=" . urlencode($anio) . "&search=" . urlencode($search) . "&pagina=" . urlencode($pagina));
            exit();
        } else {
...
<body>
    ...
    <div class="form-container">
        ...
        <a class="cancelar" href="../mostrarRegistros3_5.php?area=<?php echo urlencode($area); ?>&clasificacion=<?php echo urlencode($clasificacion); ?>&anio=<?php echo urlencode($anio); ?>&search=<?php echo urlencode($search); ?>&pagina=<?php echo urlencode($pagina); ?>">Cancelar</a>
    </div>
