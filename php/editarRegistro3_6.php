...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $en_contra_de = isset($_POST['en_contra_de']) ? trim($_POST['en_contra_de']) : '';
    ...
    if (empty($en_contra_de)) {
        $errores[] = "el en_contra_de es obligatoria.";
    }
    ...
    if (count($errores) === 0) {
        ...
        if ($stmt_update->execute()) {
            ...
            header("Location: ../mostrarRegistros3_6.php?mensaje=Registro%20actualizado%20correctamente&area=" . urlencode($area) . "&clasificacion=" . urlencode($clasificacion) . "&anio=" . urlencode($anio) . "&search=" . urlencode($search) . "&pagina=" . urlencode($pagina));
            exit();
        } else {
...
<body>
    ...
    <div class="form-container">
        ...
        <form method="post" action="" onsubmit="convertirAMayusculas()">
            ...
            <input type="text" name="en_contra_de" id="en_contra_de" spellcheck="true" value="<?php echo htmlspecialchars($registro['en_contra_de']); ?>">
        ...
        <a class="cancelar" href="../mostrarRegistros3_6.php?area=<?php echo urlencode($area); ?>&clasificacion=<?php echo urlencode($clasificacion); ?>&anio=<?php echo urlencode($anio); ?>&search=<?php echo urlencode($search); ?>&pagina=<?php echo urlencode($pagina); ?>">Cancelar</a>
    </div>
