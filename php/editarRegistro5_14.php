...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (count($errores) === 0) {
        ...
        if ($stmt_update->execute()) {
            ...
            header("Location: ../mostrarRegistros5_14.php?mensaje=Registro%20actualizado%20correctamente&area=" . urlencode($area) . "&clasificacion=" . urlencode($clasificacion) . "&anio=" . urlencode($anio) . "&search=" . urlencode($search) . "&pagina=" . urlencode($pagina));
            exit();
        } else {
...
<body>
    ...
    <form method="post" action="" onsubmit="convertirAMayusculas()">
        ...
        <label for="informacion">INFORMACIÓN
            <span class="tooltip">?
                <span class="tooltip-text">La denominación asignada a los datos que por su relevancia están respaldados (respaldo de videos de cámaras de seguridad,
                respaldo de sistema contable y manuales de operación).</span>
            </span>
        </label>
        <input type="text" name="informacion" id="informacion" value="<?php echo htmlspecialchars($registro['informacion']); ?>" required>

        <label for="medio">MEDIO
            <span class="tooltip">?
                <span class="tooltip-text">El mecanismo u objeto en el que se tiene respaldada la información. Ejemplo: Disco duro externo, servidor, CD, DVD</span>
            </span>
        </label>
        ...
        <label for="clave">CLAVE
            <span class="tooltip">?
                <span class="tooltip-text">La identificación numérica o alfanumérica asignada a los medios en los que se respaldó la información.</span>
            </span>
        </label>
        ...
        <label for="cantidad">CANTIDAD
            <span class="tooltip">?
                <span class="tooltip-text">El número de medios en los que se respaldó la información. </span>
            </span>
        </label>
        ...
        <label for="observaciones">OBSERVACIONES
            <span class="tooltip">?
                <span class="tooltip-text">Los comentarios que se consideren importantes respecto a la información respaldada.</span>
            </span>
        </label>
    ...
    <a class="cancelar" href="../mostrarregistros5_14.php?area=<?php echo urlencode($area); ?>&clasificacion=<?php echo urlencode($clasificacion); ?>&anio=<?php echo urlencode($anio); ?>&search=<?php echo urlencode($search); ?>&pagina=<?php echo urlencode($pagina); ?>">Cancelar</a>
</div>
