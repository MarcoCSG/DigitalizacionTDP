...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', '1.3 Relación de Entidades Paramunicipales'), 0, 1, 'C');
        $this->Ln(10);

        // Definir la fuente del encabezado de la tabla
        $this->SetFont('Arial', 'B', 8);
        $this->SetFillColor(192, 192, 192); // Color gris
        // Definir anchos de columnas ajustados al ancho de la página no_acta debe de tener 267
        $ancho_columnas = [
            'No.' => 10,
            'ENTIDAD PARAMUNICIPAL' => 40,
            'No. DE ACTA' => 35,
            'FECHA DE APROBACIÓN' => 37,
            'No. DE GACETA' => 40,
            'FECHA DE PUBLICACIÓN' => 40,
            'NOMBRE' => 35,
            'CARGO' => 30,
        ];
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    $query = "SELECT f.id, f.municipio, f.anio, f.ruta_archivo, 
                    f13.no, f13.entidad, f13.no_acta, f13.fecha_aprobacion, 
                    f13.no_gaceta, f13.fecha_publicacion, f13.nombre, f13.cargo, f13.informacion_al, f13.responsable
                FROM formatos f
                JOIN formato_1_3 f13 ON f.id = f13.formato_id
                WHERE f.municipio = ? AND f.anio = ?";

    if ($area_id !== null) $query .= " AND f.area_id = ?";
    if ($clasificacion_id !== null) $query .= " AND f.clasificaciones_id = ?";
    if (!empty($search)) {
        $query .= " AND (f13.no LIKE ? OR f13.entidad LIKE ?)";
        $search_param = "%" . $search . "%";
        $params[] = $search_param;
        $params[] = $search_param;
        $types .= "ss";
...
$ancho_entidad = 40;
$ancho_no_acta = 35;
$ancho_fecha_aprobacion = 37;
$ancho_no_gaceta = 40;
$ancho_fecha_publicacion = 40;
$ancho_nombre = 35;
$ancho_cargo = 30;
...
{
    global $ancho_no, $ancho_entidad, $ancho_no_acta, $ancho_fecha_aprobacion, $ancho_no_gaceta, $ancho_fecha_publicacion, $ancho_nombre, $ancho_cargo;

    // Imprimir las celdas con contenido centrado verticalmente y bordes
    PrintCenteredCell($pdf, $ancho_no, $altura_maxima, $row['no']);
    PrintCenteredCell($pdf, $ancho_entidad, $altura_maxima, $row['entidad']);
    PrintCenteredCell($pdf, $ancho_no_acta, $altura_maxima, $row['no_acta']);
    PrintCenteredCell($pdf, $ancho_fecha_aprobacion, $altura_maxima, $row['fecha_aprobacion']);
    PrintCenteredCell($pdf, $ancho_no_gaceta, $altura_maxima, $row['no_gaceta']);
    PrintCenteredCell($pdf, $ancho_fecha_publicacion, $altura_maxima, $row['fecha_publicacion']);
    PrintCenteredCell($pdf, $ancho_nombre, $altura_maxima, $row['nombre']);
    PrintCenteredCell($pdf, $ancho_cargo, $altura_maxima, $row['cargo']);
...
function ImprimirSeccionFirmas($pdf, $info_al, $responsable, $observaciones, $elaboro, $autorizo, $superviso)
{
    ...
    // Definir el ancho no_acta de la página (270mm para A4 horizontal)
    $ancho_no_acta = 270;
    $margen = 10; // Márgenes de la página
    $ancho_disponible = $ancho_no_acta - 2 * $margen; // Ancho utilizable
...
if (!is_array($datos) || count($datos) === 0) {
    ...
    for ($i = 0; $i < 2; $i++) {
        $pdf->Cell(55, $altura_fila, '', 1, 0, 'C'); // Celda vacía (ajusta el ancho según tu tabla)
        $pdf->Cell(52, $altura_fila, '', 1, 0, 'C');
        $pdf->Cell(35, $altura_fila, '', 1, 0, 'C');
        $pdf->Cell(45, $altura_fila, '', 1, 0, 'C');
        $pdf->Cell(50, $altura_fila, '', 1, 0, 'C'); // Ajusta según el número de columnas
        $pdf->Cell(30, $altura_fila, '', 1, 0, 'C');
        $pdf->Ln(); // Salto de línea para la siguiente fila
    }
}
// Calcular el número de filas
$no_acta_filas = count($datos);
...
$espacio_no_acta_disponible = 190; // Altura no_acta de la página sin márgenes
...
foreach ($datos as $row) {
    ...
    if ($pdf->GetY() + $altura_maxima > $espacio_no_acta_disponible) {
...
if ($no_acta_filas > 2 || ($pdf->GetY() + $altura_seccion_firmas > $espacio_no_acta_disponible)) {
