...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', '3.1 Relación de Reglamentos Municipales'), 0, 1, 'C');
        $this->Ln(10);
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    $query = "SELECT f.id, f.municipio, f.anio, f.ruta_archivo, 
                    f31.no, f31.denominacion, f31.no_acta, f31.fecha_aprobacion, 
                    f31.no_gaceta, f31.fecha_publicacion, f31.observaciones_3_1, f31.informacion_al, f31.responsable
                FROM formatos f
                JOIN formato_3_1 f31 ON f.id = f31.formato_id
                WHERE f.municipio = ? AND f.anio = ?";

    if ($area_id !== null) $query .= " AND f.area_id = ?";
    if ($clasificacion_id !== null) $query .= " AND f.clasificaciones_id = ?";
    if (!empty($search)) {
        $query .= " AND (f31.no LIKE ? OR f31.denominacion LIKE ?)";
        $search_param = "%" . $search . "%";
        $params[] = $search_param;
        $params[] = $search_param;
        $types .= "ss";
...
$ancho_no = 10;
$ancho_denominacion = 50;
$ancho_no_acta = 35;
$ancho_fecha_aprobacion = 47;
$ancho_no_gaceta = 40;
$ancho_fecha_publicacion = 50;
$ancho_observaciones_3_1 = 35;
...
{
    global $ancho_no, $ancho_denominacion, $ancho_no_acta, $ancho_fecha_aprobacion, $ancho_no_gaceta, $ancho_fecha_publicacion, $ancho_observaciones_3_1, $ancho_cargo;
    ...
    PrintCenteredCell($pdf, $ancho_observaciones_3_1, $altura_maxima, $row['observaciones_3_1']);
