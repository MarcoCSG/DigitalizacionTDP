...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', '3.2 Relación de Libros de Actas de Cabildo'), 0, 1, 'C');
        $this->Ln(10);

        // Definir la fuente del encabezado de la tabla
        $this->SetFont('Arial', 'B', 8);
        $this->SetFillColor(192, 192, 192); // Color gris
        // Definir anchos de columnas ajustados al ancho de la página acta_del debe de tener 267
        $ancho_columnas = [
            'No.' => 20,
            'LIBRO' => 55,
            'ACTA DEL' => 40,
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    $query = "SELECT f.id, f.municipio, f.anio, f.ruta_archivo, 
                    f32.no, f32.libro, f32.acta_del, f32.acta_al, 
                    f32.fojas, f32.observaciones_3_2, f32.informacion_al, f32.responsable
                FROM formatos f
...
{
    global $ancho_no, $ancho_libro, $ancho_acta_del, $ancho_acta_al, $ancho_fojas, $ancho_observaciones_3_2, $ancho_cargo;

    // Imprimir las celdas con contenido centrado verticalmente y bordes
    PrintCenteredCell($pdf, $ancho_no, $altura_maxima, $row['no']);
    PrintCenteredCell($pdf, $ancho_libro, $altura_maxima, $row['libro']);
    PrintCenteredCell($pdf, $ancho_acta_del, $altura_maxima, $row['acta_del']);
    PrintCenteredCell($pdf, $ancho_acta_al, $altura_maxima, $row['acta_al']);
    PrintCenteredCell($pdf, $ancho_fojas, $altura_maxima, $row['fojas']);
    PrintCenteredCell($pdf, $ancho_observaciones_3_2, $altura_maxima, $row['observaciones_3_2']);
...
function ImprimirSeccionFirmas($pdf, $info_al, $responsable, $observaciones, $elaboro, $autorizo, $superviso)
{
    ...
    $pdf->SetFont('Arial', 'B', 11);
    $pdf->Cell(40, 8, iconv('UTF-8', 'ISO-8859-1', 'LIBROS ANTERIORES:'), 0, 1, 'L');
    $pdf->SetFont('Arial', '', 11);
