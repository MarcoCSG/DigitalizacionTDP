...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', '3.3 Relación de Acuerdos de Cabildo Pendientes de Cumplir'), 0, 1, 'C');
        $this->Ln(10);

        // Definir la fuente del encabezado de la tabla
        $this->SetFont('Arial', 'B', 8);
        $this->SetFillColor(192, 192, 192); // Color gris
        // Definir anchos de columnas ajustados al ancho de la página fecha debe de tener 267
        $ancho_columnas = [
            'No.' => 10,
            'ACTA' => 45,
            'FECHA' => 30,
    ...
    if ($clasificacion_codigo !== null) {
        ...
        $stmt_clas->close();
            ...
            $query = "SELECT f.id, f.municipio, f.anio, f.ruta_archivo, 
             f33.no, f33.acta, f33.fecha, f33.acuerdo, 
             f33.responsable_cabildo, f33.estado, f33.observaciones3_3, f33.informacion_al, f33.responsable
         FROM formatos f
...
$ancho_fecha = 30;
$ancho_acuerdo = 40;
$ancho_responsable_cabildo = 40;
$ancho_estado = 37;
$ancho_observaciones3_3 = 65;
...
{
    global $ancho_no, $ancho_acta, $ancho_fecha, $ancho_acuerdo, $ancho_responsable_cabildo, $ancho_estado, $ancho_observaciones3_3, $ancho_cargo;

    // Imprimir las celdas con contenido centrado verticalmente y bordes
    PrintCenteredCell($pdf, $ancho_no, $altura_maxima, $row['no']);
    PrintCenteredCell($pdf, $ancho_acta, $altura_maxima, $row['acta']);
    PrintCenteredCell($pdf, $ancho_fecha, $altura_maxima, $row['fecha']);
    PrintCenteredCell($pdf, $ancho_acuerdo, $altura_maxima, $row['acuerdo']);
    PrintCenteredCell($pdf, $ancho_responsable_cabildo, $altura_maxima, $row['responsable_cabildo']);
    PrintCenteredCell($pdf, $ancho_estado, $altura_maxima, $row['estado']);
    PrintCenteredCell($pdf, $ancho_observaciones3_3, $altura_maxima, $row['observaciones3_3']);
...
function ImprimirSeccionFirmas($pdf, $info_al, $responsable, $observaciones, $elaboro, $autorizo, $superviso)
{
    ...
    // Definir el ancho acta_del de la página (270mm para A4 horizontal)
    $ancho_acta_del = 270;
    $margen = 10; // Márgenes de la página
    $ancho_disponible = $ancho_acta_del - 2 * $margen; // Ancho utilizable
...
$espacio_acta_del_disponible = 190; // Altura acta_del de la página sin márgenes
...
foreach ($datos as $row) {
    ...
    if ($pdf->GetY() + $altura_maxima > $espacio_acta_del_disponible) {
...
if ($acta_del_filas > 2 || ($pdf->GetY() + $altura_seccion_firmas > $espacio_acta_del_disponible)) {
