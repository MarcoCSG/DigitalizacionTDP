...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $title = '3.7 Relación de Juicios en Proceso Promovidos en contra del Ayuntamiento';
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    $query = "SELECT f.id, f.municipio, f.anio, f.ruta_archivo, 
                    f37.no, f37.tipo, f37.fecha, f37.promovido_ante, 
                    f37.demandante, f37.descripcion, f37.estado_juridico, f37.monto, 
                    f37.acciones_inmediatas, f37.informacion_al, f37.responsable
                FROM formatos f
    ...
    if (!empty($search)) {
        $query .= " AND (f37.tipo LIKE ? OR f37.responsable LIKE ?)";
        $search_param = "%" . $search . "%";
        $params[] = $search_param;
        $params[] = $search_param;
...
$ancho_tipo = 20;
$ancho_fecha = 30;
$ancho_promovido_ante = 30;
$ancho_demandante = 40;
$ancho_descripcion = 40;
$ancho_estado_juridico = 30;
$ancho_monto = 30;
$ancho_acciones_inmediatas = 37;
...
{
    global $ancho_no, $ancho_tipo, $ancho_fecha, $ancho_promovido_ante, $ancho_demandante, $ancho_descripcion, $ancho_estado_juridico, $ancho_monto,  $ancho_acciones_inmediatas;

    // Imprimir las celdas con contenido centrado verticalmente y bordes
    PrintCenteredCell($pdf, $ancho_no, $altura_maxima, $row['no']);
    PrintCenteredCell($pdf, $ancho_tipo, $altura_maxima, $row['tipo']);
    PrintCenteredCell($pdf, $ancho_fecha, $altura_maxima, $row['fecha']);
    PrintCenteredCell($pdf, $ancho_promovido_ante, $altura_maxima, $row['promovido_ante']);
    PrintCenteredCell($pdf, $ancho_demandante, $altura_maxima, $row['demandante']);
    PrintCenteredCell($pdf, $ancho_descripcion, $altura_maxima, $row['descripcion']);
    PrintCenteredCell($pdf, $ancho_estado_juridico, $altura_maxima, $row['estado_juridico']);
    PrintCenteredCell($pdf, $ancho_monto, $altura_maxima, $row['monto']);
    PrintCenteredCell($pdf, $ancho_acciones_inmediatas, $altura_maxima, $row['acciones_inmediatas']);
