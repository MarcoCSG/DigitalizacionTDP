...
class PDF extends FPDF
{
    ...
    function Header()
    {
       ...
       //$this->Image('../img/logoTDP.png', $this->GetPageWidth() - 20 - $logo_ancho, 18, $logo_ancho, $logo_alto); // Esquina superior derecha
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', '4.38 INVENTARIO DE FORMAS VALORADAS'), 0, 1, 'C');
        $this->Ln(10);
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    $query = "SELECT f.id, f.municipio, f.anio, f.ruta_archivo, 
                    f38.concepto, f38.serie, f38.del_folio, f38.al_folio, 
                    f38.area_responsable, f38.observaciones, f38.informacion_al, f38.responsable
                FROM formatos f
...
function PrintUniformRow($pdf, $row, $altura_maxima) {
    global $ancho_concepto, $ancho_serie, $ancho_del_folio, $ancho_al_folio, $ancho_area_responsable, $ancho_observaciones;
    ...
    PrintCenteredCell($pdf, $ancho_serie, $altura_maxima, $row['serie']);
    PrintCenteredCell($pdf, $ancho_del_folio, $altura_maxima, $row['del_folio']);
    PrintCenteredCell($pdf, $ancho_al_folio, $altura_maxima, $row['al_folio']);
...
if (!is_array($datos) || count($datos) === 0) {
    ...
    for ($i = 0; $i < 2; $i++) {
        $pdf->Cell(45, $altura_fila, '', 1, 0, 'C'); // Celda vacía (ajusta el ancho según tu tabla)
