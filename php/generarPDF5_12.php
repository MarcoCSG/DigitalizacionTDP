...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', '5.12 Relación de Archivo de Concentración'), 0, 1, 'C');
        $this->Ln(10);
       ...
       // Definir anchos de columnas ajustados al ancho de la página
        $ancho_columnas = [
            'No.' => 10,  
            'UNIDAD ADMINISTRATIVA' => 50,
            'SERIE DOCUMENTAL' => 35,
            'CLAVE' => 25,
            'DESCRIPCIÓN DEL CONTENIDO DE LA SERIE' => 75,
            'VIGENCIA DOCUMENTAL' => 42,
            'UBICACIÓN' => 30,    
        ];
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    $query = "SELECT f.id, f.municipio, f.anio, f.ruta_archivo, 
            f512.no, f512.unidad_administrativa, f512.serie_documental, f512.clave, f512.descripcion, f512.vigencia_documental, f512.ubicacion,
            f512.informacion_al, f512.responsable
        FROM formatos f
        JOIN formato_5_12 f512 ON f.id = f512.formato_id
        WHERE f.municipio = ? AND f.anio = ?";

    if ($area_id !== null) $query .= " AND f.area_id = ?";
    if ($clasificacion_id !== null) $query .= " AND f.clasificaciones_id = ?";
    if (!empty($search)) {
        $query .= " AND (f512.unidad_administrativa LIKE ? OR f512.serie_documental LIKE ?)";
        $search_param = "%" . $search . "%";
        $params[] = $search_param;
        $params[] = $search_param;
        $types .= "ss";
...
$ancho_no = 10;
$ancho_unidad_administrativa = 50;
$ancho_serie_documental = 35;
$ancho_clave = 25;
$ancho_descripcion = 75;
$ancho_vigencia_documental = 42;
$ancho_ubicacion = 30;
...
function PrintUniformRow($pdf, $row, $altura_maxima) {
    global $ancho_no, $ancho_unidad_administrativa, $ancho_serie_documental, $ancho_clave, $ancho_descripcion, $ancho_vigencia_documental, $ancho_ubicacion;
    
    // Imprimir las celdas con contenido centrado verticalmente y bordes
    PrintCenteredCell($pdf, $ancho_no, $altura_maxima, $row['no']);
    PrintCenteredCell($pdf, $ancho_unidad_administrativa, $altura_maxima, $row['unidad_administrativa']);
    PrintCenteredCell($pdf, $ancho_serie_documental, $altura_maxima, $row['serie_documental']);
    PrintCenteredCell($pdf, $ancho_clave, $altura_maxima, $row['clave']);
    PrintCenteredCell($pdf, $ancho_descripcion, $altura_maxima, $row['descripcion']);
    PrintCenteredCell($pdf, $ancho_vigencia_documental, $altura_maxima, $row['vigencia_documental']);
    PrintCenteredCell($pdf, $ancho_ubicacion, $altura_maxima, $row['ubicacion']);
...
$altura_seccion_firmas = 40; // Ajusta este vigencia_documental según el tamaño real de la sección de firmas
...
function CalcularAlturaCelda($pdf, $texto, $ancho_celda)
{
    ...
    $altura = ceil($lineas) * 8; // Ajusta el multiplicador para controlar el espacio entre líneas
    return $altura;
}
