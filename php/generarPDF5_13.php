...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', '5.13 Relación de Archivo Histórico'), 0, 1, 'C');
        $this->Ln(10);
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    if (!empty($search)) {
        $query .= " AND (f513.descripcion, LIKE ? OR f513.serie LIKE ?)";
        $search_param = "%" . $search . "%";
        $params[] = $search_param;
        $params[] = $search_param;
        $types .= "ss";
...
{
    global $ancho_no, $ancho_serie, $ancho_clave, $ancho_descripcion, $ancho_observaciones, $ancho_ubicacion;
