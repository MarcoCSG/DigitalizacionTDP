...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', '5.14 Relación de Archivos Electrónicos'), 0, 1, 'C');
        $this->Ln(10);
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    if (!empty($search)) {
        $query .= " AND (f514.informacion LIKE ? OR f514.medio LIKE ?)";
        $search_param = "%" . $search . "%";
        $params[] = $search_param;
        $params[] = $search_param;
        $types .= "ss";
...
function PrintUniformRow($pdf, $row, $altura_maxima) {
    global $ancho_no, $ancho_informacion, $ancho_medio, $ancho_clave, $ancho_cantidad, $ancho_observaciones;
    
    // Imprimir las celdas con contenido centrado verticalmente y bordes
    PrintCenteredCell($pdf, $ancho_no, $altura_maxima, $row['no']);
    PrintCenteredCell($pdf, $ancho_informacion, $altura_maxima, $row['informacion']);
    PrintCenteredCell($pdf, $ancho_medio, $altura_maxima, $row['medio']);
