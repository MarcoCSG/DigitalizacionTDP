...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $title = '5.15 Relación de Seguros Contratados';
        ...
        $this->Cell($ancho_columnas['Poliza'], 20, iconv('UTF-8', 'ISO-8859-1', 'POLIZA'), 1, 0, 'C', true);
        $this->Cell($ancho_columnas['Aseguradora'], 20, iconv('UTF-8', 'ISO-8859-1', 'ASEGURADORA'), 1, 0, 'C', true);

        // Celda de "VIGENCIA" combinada
        $this->Cell($ancho_columnas['del_al'], 20, '', 1, 0, 'C', true);

        // Texto "VIGENCIA" en la parte superior
        $this->Text(130, $this->GetY() + 6, 'VIGENCIA');

        // Subencabezados "DEL" y "AL"
        $this->Text(123, $this->GetY() + 15, 'DEL'); // Ajustar posición de "DEL"
        $this->Text(143, $this->GetY() + 15, 'AL');  // Ajustar posición de "AL"

        // Línea divisoria horizontal entre "VIGENCIA" y los subencabezados
        $x1 = 115;                             // Inicio de la celda "VIGENCIA"
        $x2 = 150 + $ancho_columnas['del_al']; // Fin de la celda "VIGENCIA"
        ...
        $x_div = 115 + ($ancho_columnas['del_al'] / 2); // Calcula la posición X de la línea divisoria
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    if (!empty($search)) {
        $query .= " AND (f515.concepto LIKE ? OR f515.responsable LIKE ?)";
        $search_param = "%" . $search . "%";
        $params[] = $search_param;
        $params[] = $search_param;
...
{
    global $ancho_no, $ancho_poliza, $ancho_aseguradora, $ancho_vigencia_del, $ancho_vigencia_al, $ancho_descripcion, $ancho_observaciones;

    // Imprimir las celdas con contenido centrado verticalmente y bordes
    PrintCenteredCell($pdf, $ancho_no, $altura_maxima, $row['no']);
    PrintCenteredCell($pdf, $ancho_poliza, $altura_maxima, $row['poliza']);
    PrintCenteredCell($pdf, $ancho_aseguradora, $altura_maxima, $row['aseguradora']);
...
function ImprimirSeccionVigencia_dels($pdf, $info_al, $responsable, $observaciones, $elaboro, $autorizo, $superviso)
{
...
ImprimirSeccionVigencia_dels($pdf, $info_al, $responsable, $observaciones, $elaboro, $autorizo, $superviso);
