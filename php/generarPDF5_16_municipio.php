...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $title = '5.16 Relación de Combinaciones de Cajas Fuertes';
        ...
        $ancho_columnas = [
            ...
            'Area516' => 45,
            'Responsable_516' => 35,
            'si_no' => 60, // Abarca Vigencia_del  y Vigencia_AL 
            'Observaciones' => 72,

        ];
        // Encabezados principales
        $this->Cell($ancho_columnas['Inventario'], 20, iconv('UTF-8', 'ISO-8859-1', 'INVENTARIO'), 1, 0, 'C', true);
        $this->Cell($ancho_columnas['Area516'], 20, iconv('UTF-8', 'ISO-8859-1', 'AREA'), 1, 0, 'C', true);
        $this->Cell($ancho_columnas['Responsable_516'], 20, iconv('UTF-8', 'ISO-8859-1', 'RESPONSABLE'), 1, 0, 'C', true);

        // Celda de "VIGENCIA" combinada
        $this->Cell($ancho_columnas['si_no'], 20, '', 1, 0, 'C', true);

        // Texto "VIGENCIA" en la parte superior
        $this->Text(170, $this->GetY() + 6, iconv('UTF-8', 'ISO-8859-1', 'COMBINACIÓN'));

        // Subencabezados "DEL" y "AL"
        $this->Text(163, $this->GetY() + 15, 'SI'); // Ajustar posición de "DEL"
        $this->Text(193, $this->GetY() + 15, 'NO');  // Ajustar posición de "AL"

        // Línea divisoria horizontal entre "VIGENCIA" y los subencabezados
        $x1 = 150;                             // Inicio de la celda "VIGENCIA"
        $x2 = 150 + $ancho_columnas['si_no']; // Fin de la celda "VIGENCIA"
        ...
        $x_div = 150 + ($ancho_columnas['si_no'] / 2); // Calcula la posición X de la línea divisoria
        ...
        //$this->Cell($ancho_columnas['Descripcion'], 20, iconv('UTF-8', 'ISO-8859-1', 'DESCRIPCIÓN'), 1, 0, 'C', true);
...
$ancho_inventario = 55;
$ancho_area516 = 45;
$ancho_responsable_516 = 35;
$ancho_combinacion_si = 30;
$ancho_combinacion_no = 30;
...
{
    global $ancho_no, $ancho_inventario, $ancho_area516, $ancho_responsable_516, $ancho_combinacion_si, $ancho_combinacion_no, $ancho_observaciones;

    // Imprimir las celdas con contenido centrado verticalmente y bordes
    PrintCenteredCell($pdf, $ancho_inventario, $altura_maxima, $row['inventario']);
    PrintCenteredCell($pdf, $ancho_area516, $altura_maxima, $row['area516']);
    PrintCenteredCell($pdf, $ancho_responsable_516, $altura_maxima, $row['responsable_516']);
    PrintCenteredCell($pdf, $ancho_combinacion_si, $altura_maxima, $row['combinacion_si']);
    PrintCenteredCell($pdf, $ancho_combinacion_no, $altura_maxima, $row['combinacion_no']);
