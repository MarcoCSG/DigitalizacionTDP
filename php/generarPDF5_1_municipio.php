...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', '5.1 EXISTENCIAS DE ALMACÉN'), 0, 1, 'C');
        $this->Ln(10);
        ...
        // Definir anchos de columnas ajustados al ancho de la página MEDIDA DE 267px
        $ancho_columnas = [
            'No.' => 25,
            'ARTÍCULO' => 72,
            'UNIDAD DE MEDIDA' => 35,
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    $query = "SELECT f.id, f.municipio, f.anio, f.ruta_archivo, 
            f51.no, f51.articulo, f51.unidad_medida, f51.existencia, 
            f51.costo_unitario, f51.importe, f51.informacion_al, f51.responsable
        FROM formatos f
    ...
    if (!empty($search)) {
        $query .= " AND (f51.articulo LIKE ? OR f51.unidad_medida LIKE ?)";
        $search_param = "%" . $search . "%";
        $params[] = $search_param;
        $params[] = $search_param;
        $types .= "ss";
...
$ancho_no = 25;
$ancho_articulo = 72;
$ancho_unidad_medida = 35;
$ancho_existencia = 35;
$ancho_costo_unitario = 40;
$ancho_importe = 60;
...
function PrintUniformRow($pdf, $row, $altura_maxima) {
    global $ancho_no, $ancho_articulo, $ancho_unidad_medida, $ancho_existencia, $ancho_costo_unitario, $ancho_importe;
 
    // Imprimir las celdas con contenido centrado verticalmente y bordes
    PrintCenteredCell($pdf, $ancho_no, $altura_maxima, $row['no']);
    PrintCenteredCell($pdf, $ancho_articulo, $altura_maxima, $row['articulo']);
    PrintCenteredCell($pdf, $ancho_unidad_medida, $altura_maxima, $row['unidad_medida']);
    PrintCenteredCell($pdf, $ancho_existencia, $altura_maxima, $row['existencia']);
    PrintCenteredCell($pdf, $ancho_costo_unitario, $altura_maxima, $row['costo_unitario']);
    PrintCenteredCell($pdf, $ancho_importe, $altura_maxima, $row['importe']);
...
if (!is_array($datos) || count($datos) === 0) {
    ...
    for ($i = 0; $i < 2; $i++) {
        ...
        $pdf->Cell(35, $altura_fila, '', 1, 0, 'C');
        $pdf->Cell(40, $altura_fila, '', 1, 0, 'C'); // Ajusta según el número de columnas
