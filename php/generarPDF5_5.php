...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', '5.5 RELACIÓN DE BIENES NO INVENTARIADOS'), 0, 1, 'C');
        $this->Ln(10);
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    $query = "SELECT f.id, f.municipio, f.anio, f.ruta_archivo, 
            f55.no, f55.clasificiacion_activo, f55.descripcion, f55.ubicacion, 
            f55.observaciones, f55.informacion_al, f55.responsable
        FROM formatos f
    ...
    if (!empty($search)) {
        $query .= " AND (f55.clasificiacion_activo LIKE ? OR f55.descripcion LIKE ?)";
        $search_param = "%" . $search . "%";
        $params[] = $search_param;
        $params[] = $search_param;
        $types .= "ss";
...
$ancho_no = 25;
$ancho_clasificiacion_activo = 72;
...
function PrintUniformRow($pdf, $row, $altura_maxima) {
    global $ancho_no, $ancho_clasificiacion_activo, $ancho_descripcion, $ancho_ubicacion, $ancho_observaciones;
    
    // Imprimir las celdas con contenido centrado verticalmente y bordes
    PrintCenteredCell($pdf, $ancho_no, $altura_maxima, $row['no']);
    PrintCenteredCell($pdf, $ancho_clasificiacion_activo, $altura_maxima, $row['clasificiacion_activo']);
