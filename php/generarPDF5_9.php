...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', '5.9 Relación de Información Relativa a Catastro'), 0, 1, 'C');
        $this->Ln(10);
        ...
        $ancho_columnas = [
            ...
            'MEDIO' => 55,
            'CANTIDAD' => 55,
            'OBSERVACIONES' => 70,                   // Proporción para 'Resguardado'
        ];
...
function obtenerDatos($conexion, $municipio, $anio, $area_nombre, $clasificacion_codigo, $search)
{
    ...
    $query = "SELECT f.id, f.municipio, f.anio, f.ruta_archivo, 
     f59.no, f59.informacion, f59.medio, f59.cantidad, 
     f59.observaciones, f59.informacion_al, f59.responsable
 FROM formatos f
 ...
 WHERE f.municipio = ? AND f.anio = ?";
    ...
    if (!empty($search)) {
        $query .= " AND (f59.informacion LIKE ? OR f59.medio LIKE ?)";
        $search_param = "%" . $search . "%";
        $params[] = $search_param;
        $params[] = $search_param;
        $types .= "ss";
...
function PrintUniformRow($pdf, $row, $altura_maxima) {
    global $ancho_no, $ancho_informacion, $ancho_medio, $ancho_cantidad, $ancho_observaciones;
