...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar'])) {
    ...
    $entidad = htmlspecialchars(trim($_POST['entidad']), ENT_QUOTES, 'UTF-8');
    ...
    if (empty($cargo)) {
        $errores[] = "El campo 'no cargo' es obligatorio.";
    }
    ...
    if (count($errores) === 0) {
        ...
        try {
            ...
            $stmt = $conexion->prepare("INSERT INTO formato_1_3 (formato_id, no, entidad, no_acta, fecha_aprobacion, no_gaceta, fecha_publicacion, nombre, cargo, informacion_al, responsable) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Error en la preparación de la consulta: " . $conexion->error);
            }
            $stmt->bind_param("iisssssssss", $formato_id, $no, $entidad, $no_acta, $fecha_aprobacion, $no_gaceta, $fecha_publicacion, $nombre, $cargo, $informacion_al, $responsable);

            if (!$stmt->execute()) {
                $stmt->close();
                throw new Exception("Error al insertar en 'formato_1_3': " . $stmt->error);
            }
            $stmt->close();
            ...
            echo "<script>alert('Datos guardados exitosamente.'); window.location.href = '../1.3_relacion_entidades_paramunicipales.php?area=" . urlencode($area) . "&clasificacion=" . urlencode($clasificacion) . "&anio=2022';</script>";
