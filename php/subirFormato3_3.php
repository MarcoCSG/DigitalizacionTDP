...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar'])) {
    ...
    $acta = htmlspecialchars(trim($_POST['acta']), ENT_QUOTES, 'UTF-8');
    ...
    $acuerdo = htmlspecialchars(trim($_POST['acuerdo']), ENT_QUOTES, 'UTF-8');
    $responsable_cabildo = htmlspecialchars(trim($_POST['responsable_cabildo']), ENT_QUOTES, 'UTF-8');
    $estado = htmlspecialchars(trim($_POST['estado']), ENT_QUOTES, 'UTF-8');
    $observaciones3_3 = htmlspecialchars(trim($_POST['observaciones3_3']), ENT_QUOTES, 'UTF-8');
    ...
    if (empty($observaciones3_3)) {
        $errores[] = "El campo 'observaciones3_3' es obligatorio.";
    }
    ...
    if (count($errores) === 0) {
        ...
        try {
            ...
            $stmt = $conexion->prepare("INSERT INTO formato_3_3 (formato_id, no, acta, fecha, acuerdo, responsable_cabildo, estado, observaciones3_3, informacion_al, responsable) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Error en la preparación de la consulta: " . $conexion->error);
            }
            $stmt->bind_param("iissssssss", $formato_id, $no, $acta, $fecha, $acuerdo, $responsable_cabildo, $estado, $observaciones3_3, $informacion_al, $responsable);

            if (!$stmt->execute()) {
                $stmt->close();
                throw new Exception("Error al insertar en 'formato_3_3': " . $stmt->error);
            }
            $stmt->close();
