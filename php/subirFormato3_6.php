...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar'])) {
    ...
    $en_contra_de = htmlspecialchars(trim($_POST['en_contra_de']), ENT_QUOTES, 'UTF-8');
    ...
    if (count($errores) === 0) {
        ...
        try {
            ...
            $stmt = $conexion->prepare("INSERT INTO formato_3_6 (formato_id, no, tipo, fecha, promovido_ante, en_contra_de, descripcion, estado_juridico, acciones_inmediatas, informacion_al, responsable) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Error en la preparación de la consulta: " . $conexion->error);
            }
            $stmt->bind_param("iisssssssss", $formato_id, $no, $tipo, $fecha, $promovido_ante, $en_contra_de, $descripcion, $estado_juridico, $acciones_inmediatas, $informacion_al, $responsable);
            ...
            echo "<script>alert('Datos guardados exitosamente.'); window.location.href = '../3.6_relacion_juicios_enProceso.php?area=" . urlencode($area) . "&clasificacion=" . urlencode($clasificacion) . "&anio=2022';</script>";
