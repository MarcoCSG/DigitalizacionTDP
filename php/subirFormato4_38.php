...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar'])) {
    ...
    $del_folio = filter_input(INPUT_POST, 'del_folio', FILTER_SANITIZE_NUMBER_INT);
    $al_folio = filter_input(INPUT_POST, 'al_folio', FILTER_SANITIZE_NUMBER_INT);
    ...
    if (count($errores) === 0) {
        ...
        try {
            ...
            $stmt = $conexion->prepare("INSERT INTO formato_4_38 (formato_id, concepto, serie, del_folio, al_folio, area_responsable, observaciones, informacion_al, responsable) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Error en la preparación de la consulta: " . $conexion->error);
            }
            $stmt->bind_param("issiissss", $formato_id, $concepto, $serie, $del_folio, $al_folio, $area_responsable, $observaciones, $informacion_al, $responsable);

            if (!$stmt->execute()) {
                $stmt->close();
                throw new Exception("Error al insertar en 'formato_4_38': " . $stmt->error);
            }
            $stmt->close();
            ...
            echo "<script>alert('Datos guardados exitosamente.'); window.location.href = '../4.38_inv_formas_valoradas.php?area=" . urlencode($area) . "&clasificacion=" . urlencode($clasificacion) . "&anio=2022';</script>";
