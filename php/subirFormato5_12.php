...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar'])) {
    ...
    $unidad_administrativa = htmlspecialchars(trim($_POST['unidad_administrativa']), ENT_QUOTES, 'UTF-8');
    ...
    if (count($errores) === 0) {
        ...
        try {
            ...
            // Insertar en la tabla 'formato_5_12'
            $stmt = $conexion->prepare("INSERT INTO formato_5_12 (formato_id, no, unidad_administrativa, serie_documental, clave, descripcion, vigencia_documental, ubicacion, informacion_al, responsable) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Error en la preparación de la consulta: " . $conexion->error);
            }
            $stmt->bind_param("iissssssss", $formato_id, $no, $unidad_administrativa, $serie_documental, $clave, $descripcion, $vigencia_documental, $ubicacion, $informacion_al, $responsable);

            if (!$stmt->execute()) {
                $stmt->close();
                throw new Exception("Error al insertar en 'formato_5_12': " . $stmt->error);
            }
            $stmt->close();
            ...
            echo "<script>alert('Datos guardados exitosamente.'); window.location.href = '../5.12_relacion_archivo_concentracion.php?area=" . urlencode($area) . "&clasificacion=" . urlencode($clasificacion) . "&anio=2022';</script>";
