...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar'])) {
    ...
    if (empty($informacion)) {
        $errores[] = "El campo 'informacion' es obligatorio.";
    }
    if (empty($medio)) {
    ...
    if (count($errores) === 0) {
        ...
        try {
            ...
            // Insertar en la tabla 'formato_5_14'
            $stmt = $conexion->prepare("INSERT INTO formato_5_14 (formato_id, no, informacion, medio, clave, cantidad, observaciones, informacion_al, responsable) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Error en la preparación de la consulta: " . $conexion->error);
            }
            $stmt->bind_param("iisssssss", $formato_id, $no, $informacion, $medio, $clave, $cantidad, $observaciones, $informacion_al, $responsable);

            if (!$stmt->execute()) {
                $stmt->close();
                throw new Exception("Error al insertar en 'formato_5_14': " . $stmt->error);
            }
            $stmt->close();
            ...
            echo "<script>alert('Datos guardados exitosamente.'); window.location.href = '../5.14_relacion_archivos_electronicos.php?area=" . urlencode($area) . "&clasificacion=" . urlencode($clasificacion) . "&anio=2022';</script>";
