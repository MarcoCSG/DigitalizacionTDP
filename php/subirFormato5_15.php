...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar'])) {
    ...
    $poliza = htmlspecialchars(trim($_POST['poliza']), ENT_QUOTES, 'UTF-8');
    ...
    $aseguradora = htmlspecialchars(trim($_POST['aseguradora']), ENT_QUOTES, 'UTF-8');
    ...
    if (empty($poliza)) {
        $errores[] = "El campo 'poliza' es obligatorio.";
    }
    ...
    if (empty($aseguradora)) {
        $errores[] = "El campo 'aseguradora' es obligatorio.";
    }
    ...
    if (count($errores) === 0) {
        ...
        try {
            ...
            $stmt = $conexion->prepare("INSERT INTO formato_5_15 (formato_id, no, poliza, vigencia_del, vigencia_al, descripcion, aseguradora, observaciones, informacion_al, responsable) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Error en la preparación de la consulta: " . $conexion->error);
            }
            $stmt->bind_param("iissssssss", $formato_id, $no, $poliza, $vigencia_del, $vigencia_al, $descripcion, $aseguradora, $observaciones, $informacion_al, $responsable);

            if (!$stmt->execute()) {
                $stmt->close();
                throw new Exception("Error al insertar en 'formato_5_15': " . $stmt->error);
            }
            $stmt->close();
            ...
            echo "<script>alert('Datos guardados exitosamente.'); window.location.href = '../5.15_relacion_seguros_contratados.php?area=" . urlencode($area) . "&clasificacion=" . urlencode($clasificacion) . "&anio=2022';</script>";
