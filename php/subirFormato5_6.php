...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar'])) {
    ...
    $tipo_bien = htmlspecialchars(trim($_POST['tipo_bien']), ENT_QUOTES, 'UTF-8');
    $propietario = htmlspecialchars(trim($_POST['propietario']), ENT_QUOTES, 'UTF-8');
    $vigencia = htmlspecialchars(trim($_POST['vigencia']), ENT_QUOTES, 'UTF-8');
    ...
    if (empty($propietario)) {
        $errores[] = "El campo 'propietario' es obligatorio.";
    }
    if (empty($vigencia)) {
        $errores[] = "El campo 'vigencia' es obligatorio.";
    }
    ...
    if (count($errores) === 0) {
        ...
        try {
            ...
            // Insertar en la tabla 'formato_5_6'
            $stmt = $conexion->prepare("INSERT INTO formato_5_6 (formato_id, no, tipo_bien, propietario, vigencia, observaciones, informacion_al, responsable) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Error en la preparación de la consulta: " . $conexion->error);
            }
            $stmt->bind_param("iissssss", $formato_id, $no, $tipo_bien, $propietario, $vigencia, $observaciones, $informacion_al, $responsable);

            if (!$stmt->execute()) {
                $stmt->close();
                throw new Exception("Error al insertar en 'formato_5_6': " . $stmt->error);
            }
            $stmt->close();
            ...
            echo "<script>alert('Datos guardados exitosamente.'); window.location.href = '../5.6_relacion_bienes_comodato_prestamo.php?area=" . urlencode($area) . "&clasificacion=" . urlencode($clasificacion) . "&anio=2022';</script>";
