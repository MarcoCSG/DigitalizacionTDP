...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar'])) {
    ...
    $actividad = htmlspecialchars(trim($_POST['actividad']), ENT_QUOTES, 'UTF-8');
    ...
    if (count($errores) === 0) {
        ...
        try {
            ...
            // Insertar en la tabla 'formato_9_1'
            $stmt = $conexion->prepare("INSERT INTO formato_9_1 (formato_id, no, actividad, fecha, observaciones, area_responsable, informacion_al, responsable) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Error en la preparación de la consulta: " . $conexion->error);
            }
            $stmt->bind_param("iissssss", $formato_id, $no, $actividad, $fecha, $observaciones, $area_responsable, $informacion_al, $responsable);

            if (!$stmt->execute()) {
                $stmt->close();
                throw new Exception("Error al insertar en 'formato_9_1': " . $stmt->error);
            }
            $stmt->close();
            ...
            echo "<script>alert('Datos guardados exitosamente.'); window.location.href = '../9.1_actividadesDeAtencion.php?area=" . urlencode($area) . "&clasificacion=" . urlencode($clasificacion) . "&anio=2022';</script>";
